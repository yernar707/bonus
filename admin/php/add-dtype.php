<?php
	...
	if (isset($_POST['id']) && isset($_POST['desc'])) {
		$query = 'INSERT INTO "dbSchema"."DiseaseType"(id, description) SELECT \''.$_POST['id'].'\', \''.$_POST['desc'].'\'
					 WHERE
					 	NOT EXISTS (
					 		SELECT id FROM "dbSchema"."DiseaseType" WHERE id = \''.$_POST['id'].'\'
					 	)';
		if (pg_query($connect, $query)) {
			?>
				<script type="text/javascript">
					fun1("Updated", function() {
						location.replace('/admin/disease-type.php');
					});
